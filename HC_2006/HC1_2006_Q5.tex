...
\item  
\[L = \prod^{n}_{i=1} \frac{e^{-\lambda}\lambda^{x}}{x!} \]

Hence $\log L = - n \lambda +  \sum^{n}_{i=1} x_i \log \lambda + \mbox{ constant }$.
Therefore
\[ \frac{d \log L}{d \lambda } = -n + \frac{\sum^{n}_{i=1} x_i}{\lambda} \]
which on setting equal to zero gives that the maximum likelihood estimate is \[\hat{\lambda} = \frac{\sum^{n}_{i=1} x_i}{n} = \bar{x}.\]. [Consideration of $\frac{d^2(\log L)}{d \lambda ^2}$  confirms that this is a maximum: \[ \frac{d^2(\log L)}{d \lambda ^2}  = \frac{\sum^{n}_{i=1} x_i}{\lambda^2} < 0.1 \]
...
\end{table}
  ...
  \[Var(\hat{\lambda}) = Var(\bar{X}) = \frac{Var(X)}{n} = \frac{\lambda}{n}.\]
...
\begin{itemize}
    ...
    \item We estimate the variance by 
$\hat{\lambda}/n$, so that we have ${ \displaystyle \hat{\lambda} \sim N( \lambda, \frac{\hat{\lambda}}{n})   }$, approximately.
...
\end{tabular}
\end{table}
\item  For the given sample, we have $n = 12$ and $\sum x_i = 484$, leading to $\hat{\lambda} = \bar{x} = 4$. The approximate confidence interval is therefore
\[ 4 \pm 1.96 \sqrt{ \frac{4}{12}} = 4 \pm 1.13 = (2.87 , 5.13).\]
